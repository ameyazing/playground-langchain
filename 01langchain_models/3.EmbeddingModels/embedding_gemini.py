...
document = ["Harry potter was a gifted wizard who studied in the Hogwarts school of witchcraft and wizardry.",
...
document_vectors = embedding_model.embed_documents(document)

similarity_scores = cosine_similarity([vector], document_vectors)
sorted_documents_scores = sorted(enumerate(similarity_scores[0]), key=lambda x: x[1], reverse=True)
print("Most relevant documents:")
for index, score in sorted_documents_scores:
    print(f"Document: {document[index]}, Similarity Score: {score}")
